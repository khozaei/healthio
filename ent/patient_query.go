...
func (pq *PatientQuery) QueryReception() *ReceptionQuery {
	query := (&ReceptionClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PatientQuery) Clone() *PatientQuery {
	...
	return &PatientQuery{
		...
		withReception:  pq.withReception.Clone(),
...
// WithReception tells the query-builder to eager-load the nodes that are connected to
// the "reception" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PatientQuery) WithReception(opts ...func(*ReceptionQuery)) *PatientQuery {
	query := (&ReceptionClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withReception = query
	return pq
}
...
func (pq *PatientQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Patient, error) {
	...
	if query := pq.withAttachment; query != nil {
		if err := pq.loadAttachment(ctx, query, nodes,
			func(n *Patient) { n.Edges.Attachment = []*Attachment{} },
			func(n *Patient, e *Attachment) { n.Edges.Attachment = append(n.Edges.Attachment, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withReception; query != nil {
		if err := pq.loadReception(ctx, query, nodes,
			func(n *Patient) { n.Edges.Reception = []*Reception{} },
			func(n *Patient, e *Reception) { n.Edges.Reception = append(n.Edges.Reception, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PatientQuery) loadAttachment(ctx context.Context, query *AttachmentQuery, nodes []*Patient, init func(*Patient), assign func(*Patient, *Attachment)) error {
	...
	query.Where(predicate.Attachment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(patient.AttachmentColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.patient_attachment
		if fk == nil {
			return fmt.Errorf(`foreign-key "patient_attachment" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "patient_attachment" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *PatientQuery) loadReception(ctx context.Context, query *ReceptionQuery, nodes []*Patient, init func(*Patient), assign func(*Patient, *Reception)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Patient)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Reception(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(patient.ReceptionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.patient_reception
		if fk == nil {
			return fmt.Errorf(`foreign-key "patient_reception" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "patient_reception" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
