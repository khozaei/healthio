...
func (vq *VisitQuery) QueryReception() *ReceptionQuery {
	query := (&ReceptionClient{config: vq.config}).Query()
...
func (vq *VisitQuery) QueryAttachment() *AttachmentQuery {
	query := (&AttachmentClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VisitQuery) Clone() *VisitQuery {
	...
	return &VisitQuery{
		...
		withReception:  vq.withReception.Clone(),
		withAttachment: vq.withAttachment.Clone(),
...
func (vq *VisitQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Visit, error) {
	var (
		...
		_spec       = vq.querySpec()
		loadedTypes = [2]bool{
			vq.withReception != nil,
			vq.withAttachment != nil,
		}
	)
	if vq.withReception != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := vq.withReception; query != nil {
		if err := vq.loadReception(ctx, query, nodes, nil,
			func(n *Visit, e *Reception) { n.Edges.Reception = e }); err != nil {
			return nil, err
		}
	}
	if query := vq.withAttachment; query != nil {
		if err := vq.loadAttachment(ctx, query, nodes,
			func(n *Visit) { n.Edges.Attachment = []*Attachment{} },
			func(n *Visit, e *Attachment) { n.Edges.Attachment = append(n.Edges.Attachment, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VisitQuery) loadReception(ctx context.Context, query *ReceptionQuery, nodes []*Visit, init func(*Visit), assign func(*Visit, *Reception)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Visit)
	for i := range nodes {
		if nodes[i].reception_visit == nil {
			continue
		}
		fk := *nodes[i].reception_visit
	...
	query.Where(reception.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "reception_visit" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (vq *VisitQuery) loadAttachment(ctx context.Context, query *AttachmentQuery, nodes []*Visit, init func(*Visit), assign func(*Visit, *Attachment)) error {
	...
	query.Where(predicate.Attachment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(visit.AttachmentColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.visit_attachment
		if fk == nil {
			return fmt.Errorf(`foreign-key "visit_attachment" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "visit_attachment" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
