...
func (aq *AttachmentQuery) QueryVisit() *VisitQuery {
	query := (&VisitClient{config: aq.config}).Query()
...
func (aq *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if aq.withVisit != nil || aq.withPatient != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withVisit; query != nil {
		if err := aq.loadVisit(ctx, query, nodes, nil,
			func(n *Attachment, e *Visit) { n.Edges.Visit = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withPatient; query != nil {
		if err := aq.loadPatient(ctx, query, nodes, nil,
			func(n *Attachment, e *Patient) { n.Edges.Patient = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttachmentQuery) loadVisit(ctx context.Context, query *VisitQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Visit)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Attachment)
	for i := range nodes {
		if nodes[i].visit_attachment == nil {
			continue
		}
		fk := *nodes[i].visit_attachment
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "visit_attachment" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AttachmentQuery) loadPatient(ctx context.Context, query *PatientQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Patient)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Attachment)
	for i := range nodes {
		if nodes[i].patient_attachment == nil {
			continue
		}
		fk := *nodes[i].patient_attachment
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "patient_attachment" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
