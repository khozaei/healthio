...
// The ReceptionFunc type is an adapter to allow the use of ordinary
// function as Reception mutator.
type ReceptionFunc func(context.Context, *ent.ReceptionMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ReceptionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ReceptionMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ReceptionMutation", m)
}
